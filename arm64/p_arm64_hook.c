...
unsigned char hook_code[] = {0xE1, 0x03, 0xBE, 0xA9, 0x40, 0x00, 0x00,
                             0x58, 0x00, 0x00, 0x1F, 0xD6, 0x00, 0x00,
                             0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

unsigned char hook_ret_code[] = {0x49, 0x00, 0x00, 0x58, 0x20, 0x01,
                                 0x1F, 0xD6, 0x00, 0x00, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00};
...
long get_adrp_target_addr(unsigned long start_addr) {
  ...
  target_addr +=
      ((unsigned long) byte_code - 9) * 0x1000 / 2 + (start_addr & (~0xfff));
...
unsigned long get_init_mm_address(void) {
  ...
  if (!start_addr) {
    ...
    if (!start_addr) {
      start_addr =
          (unsigned long) P_SYM(p_kallsyms_lookup_name)("__pte_alloc_kernel");
      if (!start_addr) {
        return 0;
      }
    }
  ...
  pos = P_SYM(p_get_symbol_pos)((unsigned long) start_addr, &symbol_size,
                                &offset);
  if (!pos || offset)
    return 0;
  //adrp
...
static inline void arm64_stub_fixup(hook_stub *stub, const void *entry,
                                    const void *ret) {
...
int inline_hook_install(void *arg) {
  ...
  arm64_stub_fixup(stub, p_current_hook_struct->entry_fn,
                   p_current_hook_struct->ret_fn);
  *(unsigned long *) &hook_code[12] = (unsigned long) stub->hook;
  *(unsigned long *) &hook_ret_code[8] =
      (unsigned long) p_current_hook_struct->addr + stub->nbytes;

  memcpy(stub->orig, p_current_hook_struct->addr, stub->nbytes);
  memcpy(stub->orig + stub->nbytes, hook_ret_code, sizeof(hook_ret_code));

#  if LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0)
  p_ret = remap_write_range(p_current_hook_struct->addr, hook_code,
                            stub->nbytes, true);
#  else
  remain =
      PAGE_SIZE - (((unsigned long) p_current_hook_struct->addr) % PAGE_SIZE);
  //Double Page
  if (remain < stub->nbytes) {
    p_ret = write_ro_memory(p_current_hook_struct->addr, hook_code, remain);
    if (p_ret != 0)
      return p_ret;
    p_ret = write_ro_memory(p_current_hook_struct->addr + remain,
                            &hook_code[remain], stub->nbytes - remain);
  } else {
    p_ret =
        write_ro_memory(p_current_hook_struct->addr, hook_code, stub->nbytes);
  }
#  endif
...
int inline_hook_uninstall(void *arg) {
  ...
  stub = p_current_hook_struct->stub;
#  if LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0)
  p_ret = remap_write_range(p_current_hook_struct->addr, stub->orig,
                            stub->nbytes, true);
#  else
  remain =
      PAGE_SIZE - (((unsigned long) p_current_hook_struct->addr) % PAGE_SIZE);
  ...
  if (remain < stub->nbytes) {
    ...
    p_ret = write_ro_memory(p_current_hook_struct->addr + remain,
                            &stub->orig[remain], stub->nbytes - remain);
  } else {
    p_ret =
        write_ro_memory(p_current_hook_struct->addr, stub->orig, stub->nbytes);
  }
#  endif
  return 0;
}
