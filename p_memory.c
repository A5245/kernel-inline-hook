...
static int change_allocate_page_range(pte_t *ptep, unsigned long addr,
                                      void *data) {
...
#  else
static int change_allocate_page_range(pte_t *ptep, pgtable_t token,
...
static int change_allocate_memory_common(unsigned long addr, int numpages,
                                         pgprot_t set_mask,
                                         pgprot_t clear_mask) {
  ...
  if (!P_SYM(p_flush_tlb_kernel_range)) {
    P_SYM(p_flush_tlb_kernel_range) =
        (void *) P_SYM(p_kallsyms_lookup_name)("flush_tlb_kernel_range");
    if (!P_SYM(p_flush_tlb_kernel_range))
      return -1;
  }

  if (!P_SYM(p_apply_to_page_range)) {
    P_SYM(p_apply_to_page_range) =
        (void *) P_SYM(p_kallsyms_lookup_name)("apply_to_page_range");
    if (!P_SYM(p_apply_to_page_range))
      return -1;
  }
  ...
  ret = P_SYM(p_apply_to_page_range)(P_SYM(p_init_mm), start, size,
                                     change_allocate_page_range, &data);
...
#  if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 8, 0) && defined(CONFIG_ARM64)
  return change_allocate_memory_common(addr, numpages, __pgprot(PTE_MAYBE_GP),
                                       __pgprot(PTE_PXN));
#  elif LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0) && defined(CONFIG_ARM64)
  return change_allocate_memory_common(addr, numpages, __pgprot(0),
                                       __pgprot(PTE_PXN));
#  endif
}
#endif